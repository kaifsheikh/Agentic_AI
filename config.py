...
class Config:
    ...
    def validate(cls):
        ...
        # Warn (don't crash) about insecure defaults when not in debug/dev mode.
        if not cls.DEBUG:
            if cls.SECRET_KEY == "your-secret-key-change-me":
                logger.warning(
            ...
            if cls.CORS_ORIGINS == "*":
                logger.warning(
                    "SECURITY WARNING: CORS_ORIGINS is '*' (any origin allowed). "
                    "Restrict this to your actual frontend domain(s) in .env for production."
                )
